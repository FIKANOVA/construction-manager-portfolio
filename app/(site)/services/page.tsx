...
import { client, queries, type ServicePackage } from '@/lib/sanity'

export const revalidate = 60 // Revalidate every minute

async function getServicePackages(): Promise<ServicePackage[]> {
    try {
        return await client.fetch(queries.allServicePackages)
    } catch (error) {
...
export default async function ServicesPage() {
    ...
    const displayServices = packages.length > 0 ? packages : fallbackServices

    // Helper to get image based on category if not provided
    const getCategoryImage = (category: string) => {
        switch (category) {
            case 'construction':
                return 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?q=80&w=2689&auto=format&fit=crop'
            case 'gis':
                return 'https://images.unsplash.com/photo-1569336415962-a4bd9f69cd83?q=80&w=2669&auto=format&fit=crop'
            case 'ai-data':
                return 'https://images.unsplash.com/photo-1620712943543-bcc4688e7485?q=80&w=2565&auto=format&fit=crop'
            default:
                return 'https://images.unsplash.com/photo-1503387762-592deb58ef4e?q=80&w=2689&auto=format&fit=crop'
        }
    }
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="space-y-16">
                    {displayServices.map((service, index) => (
                        <ScrollReveal key={service._id} delay={index * 0.1}>
                ...
                <ScrollReveal delay={0.3}>
                    <div className="mt-16 text-center py-12 border border-white/10 bg-white/[0.02]">
                        <h3 className="text-2xl font-light tracking-wide mb-4 text-white">
                        ...
                        <Link
                            href="/contact"
                            className="inline-block px-8 py-3 border border-white/30 text-white text-sm tracking-[0.15em] uppercase hover:bg-white hover:text-black transition-all duration-300"
                        >
                            Get in Touch
                        </Link>
                    </div>
